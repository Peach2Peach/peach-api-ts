...
type SearchOffersRequestQuery = { page?: number; size?: number; sortBy?: Sorter[] }
...
export const searchOffers =
  ...
  async ({
    ...
    const response = await fetch(`${url}/v1/offer/search?page=${page}&size=${size}&sortBy=${sortBy.join(',')}`, {
    ...
    return parseResponse<SearchOffersResponseBody, SearchOffersErrorResponseBody>(response)
  }
