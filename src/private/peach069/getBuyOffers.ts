...
export type GetBuyOfferRequestQuery = {minAmountSats?:number, maxAmountSats?:number , ownOffers?:boolean};
...
export const getBuyOffers =
  ...
  async ({    minAmountSats, maxAmountSats,ownOffers
  }: Props) => {

    const searchParams = new URLSearchParams();
    for (const [key, value] of Object.entries({    minAmountSats, maxAmountSats,ownOffers
  })) {
    if (value !== undefined) {
