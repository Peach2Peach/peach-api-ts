...
export type GetSellOfferRequestQuery = {minAmountSats?:number, maxAmountSats?:number };
...
export const getSellOffers =
  ...
  async ({    minAmountSats, maxAmountSats
  }: Props) => {

    const searchParams = new URLSearchParams();
    for (const [key, value] of Object.entries({    minAmountSats, maxAmountSats
  })) {
    if (value !== undefined) {
